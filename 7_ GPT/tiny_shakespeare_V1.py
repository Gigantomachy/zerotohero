...
#     context = x[:t+1]
#     target = y[t] # character ahead of x
...
class BigramLanguageModel(nn.Module):
    def __init__(self, vocab_size):
        ...
        # this is currently a *bigram* model to start with, so unlike earlier lectures, here we only have
        # one embedding -> which represents the probability of the next character given a character
        self.token_embedding_table = nn.Embedding(vocab_size, vocab_size)

    def forward(self, idx, targets=None):
        # idx and targets are both (B,T) tensor of integers
        # refresher: logits are scores for the possible next character
        logits = self.token_embedding_table(idx) # (B,T,C) batch size, context window, 65 scores
        if targets is None:
            loss = None
        else:
            print(logits.shape)
